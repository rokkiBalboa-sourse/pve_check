...
check_task "$HQ_SRV" "RAID0 /dev/md4" \
    "grep -q 'md0' /proc/mdstat" \
    "grep md0 /proc/mdstat | head -1"
...
check_task "$ISP" "Chrony сервер" \
    "grep -q 'local stratum 9' /etc/chrony.conf" \
...
check_task "$HQ_RTR" "DNAT 2014->192.168.1.10" \
...
check_task "$ISP" "web proxy -> 172.16.1.10:8084" \
    "grep -q 'proxy_pass http://172.16.1.10:8084' /etc/nginx/sites-enabled.d/r-proxy.conf 2>/dev/null" \
    "grep 'proxy_pass http://172.16.1.10:8084' /etc/nginx/sites-enabled.d/r-proxy.conf 2>/dev/null || echo 'not_found'"
check_task "$ISP" "docker proxy -> 172.16.2.10:8084" \
