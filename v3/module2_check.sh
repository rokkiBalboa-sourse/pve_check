...
check_task "$HQ_SRV" "Монтирование /raid" \
    "mount | grep -q '/dev/md3 on /raid'" \
    "mount | grep '/dev/md3' | awk '{print \$3}'"
...
check_task "$ISP" "Chrony сервер" \
    "grep -q 'local stratum 8' /etc/chrony.conf" \
...
check_task "$HQ_RTR" "DNAT 2013->192.168.1.10" \
    "nft list ruleset 2>/dev/null | grep -q 'dport 2013 dnat to 192.168.1.10'" \
    "nft list ruleset 2>/dev/null | grep 'dport 2013' | grep -oP 'dnat to \K[^ ]+' | tr -d ' ' | head -1 || echo 'not_found'"

check_task "$HQ_RTR" "DNAT 8083->192.168.1.10:80" \
    "nft list ruleset 2>/dev/null | grep -q 'dport 8083 dnat to 192.168.1.10:80'" \
    "nft list ruleset 2>/dev/null | grep 'dport 8083' | grep -oP 'dnat to \K[^ ]+' | tr -d ' ' | head -1 || echo 'not_found'"

check_task "$BR_RTR" "DNAT {8083,2013}->192.168.3.10" \
    "nft list ruleset 2>/dev/null | grep -E 'dport.*\{.*(8083|2013).*\}' | grep -q 'dnat to 192.168.3.10'" \
    "nft list ruleset 2>/dev/null | grep -E 'dport.*\{.*(8083|2013).*\}' | grep -oP 'dnat to \K[0-9.]+' | tr -d ' ' | head -1 || echo 'not_found'"
...
check_task "$ISP" "web proxy -> 172.16.1.10:8083" \
    "grep -q 'proxy_pass http://172.16.1.10:8083' /etc/nginx/sites-enabled.d/r-proxy.conf 2>/dev/null" \
    "grep 'proxy_pass http://172.16.1.10:8083' /etc/nginx/sites-enabled.d/r-proxy.conf 2>/dev/null || echo 'not_found'"
check_task "$ISP" "docker proxy -> 172.16.2.10:8083" \
    "grep -q 'proxy_pass http://172.16.2.10:8083' /etc/nginx/sites-enabled.d/r-proxy.conf 2>/dev/null" \
    "grep 'proxy_pass http://172.16.2.10:8083' /etc/nginx/sites-enabled.d/r-proxy.conf 2>/dev/null || echo 'not_found'"
check_task "$ISP" "Basic auth WEB" \
    "test -f /etc/nginx/.htpasswd && grep -q 'Kazimir' /etc/nginx/.htpasswd" \
