...
# BR-RTR
check_ip "$BR_RTR" "enp7s1" "172.16.20.2/28"
check_ip "$BR_RTR" "enp7s2" "192.168.0.1/28"
# BR-SRV
check_ip "$BR_SRV" "enp7s1" "192.168.0.2/28"
# HQ-RTR
check_ip "$HQ_RTR" "enp7s1" "172.16.10.2/28"
...
check_ip "$ISP" "enp7s2" "172.16.10.1/28"
check_ip "$ISP" "enp7s3" "172.16.20.1/28"
...
log_report "Проверка HQ-SRV (net6, tag=111)..."
check_vm_vlan_net6 "$HQ_SRV" "111"

log_report "Проверка HQ-CLI (net6, tag=211)..."
check_vm_vlan_net6 "$HQ_CLI" "211"
...
check_ssh_port "$HQ_SRV" "2011"
check_ssh_port "$BR_SRV" "2011"
...
check_ssh_connection "$HQ_RTR" "192.168.100.2" "2011" "sshuser"
check_ssh_connection "$BR_RTR" "192.168.0.2" "2011" "sshuser"
...
check_gre_tunnel "$HQ_RTR" "gre1" "172.16.10.2" "172.16.20.2"
check_gre_tunnel "$BR_RTR" "gre1" "172.16.20.2" "172.16.10.2"
...
check_dhcp_config "$HQ_RTR" "dhcp-range=interface:vlan211,192.168.200.2,192.168.200.2,255.255.255.240,6h"
check_task "$HQ_RTR" "DNS перенаправлен на 192.168.100.2" "grep -q 'nameserver 192.168.100.2' /etc/net/ifaces/vlan111/resolv.conf" "cat /etc/net/ifaces/vlan111/resolv.conf 2>/dev/null || echo 'not_set'"
