...
check_task "$HQ_SRV" "RAID5 /dev/md0" \
    "grep -q 'md2' /proc/mdstat" \
    "grep md2 /proc/mdstat | head -1"
check_task "$HQ_SRV" "Монтирование /raid" \
    "mount | grep -q '/dev/md0 on /raid'" \
    "mount | grep '/dev/md0' | awk '{print \$3}'"
...
check_task "$ISP" "Chrony сервер" \
    "grep -q 'local stratum 7' /etc/chrony.conf" \
...
check_task "$HQ_RTR" "DNAT 2026->192.168.1.10" \
    "nft list ruleset 2>/dev/null | grep -q 'dport 2012 dnat to 192.168.1.10'" \
    "nft list ruleset 2>/dev/null | grep 'dport 2012' | grep -oP 'dnat to \K[^ ]+' | tr -d ' ' | head -1 || echo 'not_found'"

check_task "$HQ_RTR" "DNAT 8080->192.168.1.10:80" \
    "nft list ruleset 2>/dev/null | grep -q 'dport 8082 dnat to 192.168.1.10:80'" \
    "nft list ruleset 2>/dev/null | grep 'dport 8082' | grep -oP 'dnat to \K[^ ]+' | tr -d ' ' | head -1 || echo 'not_found'"

check_task "$BR_RTR" "DNAT {8082,2012}->192.168.3.10" \
    "nft list ruleset 2>/dev/null | grep -E 'dport.*\{.*(8082|2012).*\}' | grep -q 'dnat to 192.168.3.10'" \
    "nft list ruleset 2>/dev/null | grep -E 'dport.*\{.*(8082|2012).*\}' | grep -oP 'dnat to \K[0-9.]+' | tr -d ' ' | head -1 || echo 'not_found'"
...
check_task "$ISP" "web proxy -> 172.16.1.10:8082" \
    "grep -q 'proxy_pass http://172.16.1.10:8082' /etc/nginx/sites-enabled.d/r-proxy.conf 2>/dev/null" \
    "grep 'proxy_pass http://172.16.1.10:8082' /etc/nginx/sites-enabled.d/r-proxy.conf 2>/dev/null || echo 'not_found'"
check_task "$ISP" "docker proxy -> 172.16.2.10:8082" \
    "grep -q 'proxy_pass http://172.16.2.10:8082' /etc/nginx/sites-enabled.d/r-proxy.conf 2>/dev/null" \
    "grep 'proxy_pass http://172.16.2.10:8082' /etc/nginx/sites-enabled.d/r-proxy.conf 2>/dev/null || echo 'not_found'"
check_task "$ISP" "Basic auth WEB" \
    "test -f /etc/nginx/.htpasswd && grep -q 'Kozmac' /etc/nginx/.htpasswd" \
